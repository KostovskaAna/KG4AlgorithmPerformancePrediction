...
def get_embeddings(model, dim, budget, fixed_target, algo):
    _k = model.k
    ...
    df_embeddings_conf.to_csv(f'../Results/embeddings/{algo}/embeddings_confs_ComplEx_dim_{dim}_budget_{budget}_target_{fixed_target:.5f}.csv')
    ...
    df_embeddings_problems.to_csv(f'../Results/embeddings/{algo}/embeddings_problems_ComplEx_dim_{dim}_budget_{budget}_target_{fixed_target:.5f}.csv')                   

def HPO_KG_embedding(dim, budget, fixed_target, algo, k, loss, lr):
    ...
    text_file = open(f"../Results/HPO/{algo}/trace_ComplEx_dim_{dim}_budget_{budget}_target_{fixed_target:.5f}.txt", "a")
            ...
            early_stopping_params = \
                    {
                        ...
                        'stop_interval':10,           # stops if 10 successive validation checks are bad.
    ...
    np.save(f"../Results/HPO/{algo}/test_preds_ComplEx_dim_{dim}_budget_{budget}_target_{fixed_target:.5f}_k_{k}_loss_{loss}_lr_{lr}.npy", y_test_preds)
    np.save(f"../Results/HPO/{algo}/test_true_ComplEx_dim_{dim}_budget_{budget}_target_{fixed_target:.5f}_k_{k}_loss_{loss}_lr_{lr}.npy", y_test)
...
if __name__ == "__main__":
    ...
    loss = sys.argv[6]
    lr = float(sys.argv[7])
    model = HPO_KG_embedding(dim, budget, fixed_target, algo, k, loss, lr)
    get_embeddings(model, dim, budget, fixed_target, algo)